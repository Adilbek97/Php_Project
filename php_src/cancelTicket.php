...
if(isset($_POST['btnCancel'])){
  ...
  $password_t=$data['password_t'];

  $link = mysqli_connect($host, $user, $password, $database) 
             or die("Ошибка к подключение базаданных " . mysqli_error($link));
             $query ="SELECT user_password FROM tickets WHERE number_of_ticket='$number_ticket'";
              $result = mysqli_query($link, $query) or die("Ошибка " . mysqli_error($link)); 
              if($result)
              {
                $rows = mysqli_num_rows($result);
                if($rows==0){
                  echo "<script>alert('такого билета нет.'');</script>";
                  }else{
                  ...
                  if($row[0]==$password_t){
                    ...
                    //  if($delete_result){
                    //       echo "<p style='color: green;'>ваш билет успешно отменено</p>";
                          
                    //       setcookie ("cancelTicket", "ваш билет(".$number_ticket.") успешно отменено");
                    //       header("Location: msgTicket.php");
                    //   }
                    setcookie('queryToСancel',$number_ticket);
                    echo "<script>alert('Ваш запрос $number_ticket отправлено');window.location='../index1.php';</script>";

                  }else{
                    echo "<script>alert('пароль почта не верно.'');</script>";
...
<br>
    ...
    <div class="container ">
      <form class="clform" action="cancelTicket.php" method="POST">
        <div class="form-group">
                <label for="email">Номер билета</label>
                <input type="text" class="form-control" id="firstName" name="number_ticket" >
              </div>
      <div class="form-group">
        <label for="email">Пароль вашего почту</label>
        <input type="password" class="form-control" id="firstName" name="password_t" >
      </div>
      
       <div class="btn">
                <button type="submit" name="btnCancel" class="btn btn-success">Отменить</button>
      </form>
