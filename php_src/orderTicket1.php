            ...
            $link = mysqli_connect($host, $user, $password, $database) 
             ...
             $query ="SELECT * FROM users WHERE mail='$mail'";
              ...
              if($result)
              {
                ...
                $insert_query = "INSERT INTO `tickets` VALUES (NULL, '$row[0]', '$row[2]', '$row[1]', '$row[7]', '$row[8]', '$from_c', '$where_c', '$clas1', '$go_when', '$go_back', '$number_of_ticket','$sena')";
                $insert_result = mysqli_query($link, $insert_query) or die("Ошибка " . mysqli_error($link)); 
                if ($insert_result) {
                  echo "<script>
                    alert('уважаемый $row[2] $row[1] ваш заказ принят nomer vashego bileta  $number_of_ticket');
